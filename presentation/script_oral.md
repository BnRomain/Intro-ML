...
« On travaille sur SmallDB, un petit jeu tiré de PASCAL VOC, avec six races : Chihuahua, basset, Kerry blue terrier, groenendael, malinois et chow. Chaque photo vient avec un fichier XML qui indique le cadre du chien.
...
« Les classifieurs ont besoin d'images de même taille : on les ramène toutes en 256 par 256. Mais si on redimensionne brutalement, le chien est écrasé ou étiré. Donc on garde les proportions et on place l'image au centre, ce qui laisse des bords vides à remplir.
On a testé trois remplissages : noir, blanc, ou continu, c'est-à-dire en prolongeant la couleur du bord. On garde le continu : le noir créerait une frontière nette qui serait prise pour un vrai contour par le HOG, ce qu'on veut éviter. Je passe la main à Romain. »
...
### Slide 6 — PCA *(1:00)*
« Merci. Une image en 256 par 256, c'est plus de 65 000 valeurs : beaucoup trop pour travailler directement. La PCA résume cette information sur bien moins d'axes.
...
On règle les paramètres par GridSearch en validation croisée : on retient C égal 5, gamma 10 puissance moins 4, et 15 axes PCA par race.
Le tableau compare les stratégies un-contre-un et un-contre-tous, en linéaire et en RBF. Le RBF bat le linéaire, logique vu que les races ne sont pas séparables par des droites. Le meilleur, un-contre-tous RBF, atteint 57,77 %. »

### Slide 12 — Confusion SVM *(0:35)*
« La matrice de confusion du meilleur SVM montre une diagonale un peu plus marquée que le kNN : certaines races sont bien reconnues. Mais il reste pas mal de confusions, entre les races qui se ressemblent. »
...
- **Pourquoi le remplissage continu et pas le noir ?** Le noir crée une bordure franche que le HOG interprète comme un contour réel, ça injecte un faux signal identique pour toutes les images.
